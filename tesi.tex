...
An important advancement in network architecture has been brought by the introduction of Shortcut (or Residual) Connections \cite{residual}. Famously, deep models suffer from {\bf degradation problems} after reaching a maximum 

instead of learning a function $F(x)$ we try to learn $F(x) + x$

During the back propagation the gradient of a higher layer can easily pass to the lower layer, without being mediated by any layer, which may cause vanishing or exploding gradient problem.
